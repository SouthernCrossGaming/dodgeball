...
#define SOUND_ALERT_VOL	0.6
#define HUD_LINE_SEPARATION 0.04

//Nuke explosion
#define PARTICLE_NUKE_1         "fireSmokeExplosion"
#define PARTICLE_NUKE_2         "fireSmokeExplosion2"
#define PARTICLE_NUKE_3         "fireSmokeExplosion3"
#define PARTICLE_NUKE_4         "fireSmokeExplosion4"
...
public void OnPluginStart()
{
    ...
    HookEvent("player_team", OnPlayerInventory);
...
void LoadRocketClasses()
{
    ...
    defClass.hitboxX = kv.GetFloat("HitboxX",0.0);
    defClass.hitboxY = kv.GetFloat("HitboxY",0.0);
    defClass.hitboxZ = kv.GetFloat("HitboxZ",0.0);

    //Sprite
    kv.GetString("Sprite",auxPath,PLATFORM_MAX_PATH,"");
    defClass.SetSprite(auxPath);
    ...
    defClass.spawnmult = kv.GetNum("SwarmMultiplier", 1);
    //Bounce
    defClass.maxbounce = kv.GetNum("MaxBounce",10);
    defClass.bouncedelay = kv.GetFloat("BounceDelay",0.1);
    ...
    if (kv.JumpToKey("explosion"))
    {
        ...
        defClass.exp_radiusinc = kv.GetFloat("RadiusSizeInc",0.0);
    ...
    do
    {
        ...
        g_RocketClass[count].hitboxX = kv.GetFloat("HitboxX",defClass.hitboxX);
        g_RocketClass[count].hitboxY = kv.GetFloat("HitboxY",defClass.hitboxY);
        g_RocketClass[count].hitboxZ = kv.GetFloat("HitboxZ",defClass.hitboxZ);

        //Sprite
        defClass.GetSprite(auxPath,PLATFORM_MAX_PATH);
        kv.GetString("Sprite",auxPath,PLATFORM_MAX_PATH,auxPath);
        g_RocketClass[count].SetSprite(auxPath);
        ...
        g_RocketClass[count].spawnmult = kv.GetNum("SwarmMultiplier",defClass.spawnmult);
        //Bounce
        g_RocketClass[count].maxbounce = kv.GetNum("MaxBounce",defClass.maxbounce);
        g_RocketClass[count].bouncedelay = kv.GetFloat("BounceDelay",defClass.bouncedelay);
        ...
        if (kv.JumpToKey("explosion"))
        {
            ...
            g_RocketClass[count].exp_radiusinc = kv.GetFloat("RadiusSizeInc",defClass.exp_radiusinc);
...
public void PrecacheFiles()
{
    ...
    for( int i = 0; i < g_RocketClass_count; i++)
    {
        ...
        g_RocketClass[i].GetSprite(auxPath,PLATFORM_MAX_PATH);
        if(!StrEqual(auxPath,""))
        {
            PrecacheModelEx(auxPath,true,true);
        }
...
public Action OnRoundEnd(Handle event, const char[] name, bool dontBroadcast)
{
    ...
    StopSoundAllDB("dodgeball/bumblest.mp3");
    ...
    for(int i = 0; i < MAXROCKETS; i++)
    {
        ...
        if (index != INVALID_ENT_REFERENCE)
        {
            ...
            if (dissolver == -1)  return Plugin_Continue;
...
public Action OnPlayerInventory(Handle event, const char[] name, bool dontBroadcast)
{
    ...
    if (wep_ent == -1 && IsPlayerAlive(client))
    {
    ...
    if(wep_ent > MaxClients && IsValidEntity(wep_ent))
    {
        ...
        LogMessage("Found weapon DEFINDEX for %N is %i", client, wep_index);
        ...
        if (wep_ent > MaxClients && IsValidEdict(wep_ent) && GetEdictClassname(wep_ent, classname, sizeof(classname)))
        {
            if (StrEqual(classname, "tf_weapon_flamethrower", false) )
            {
                ...
                if(!replace_primary)
                {
                    LogMessage("Attribute manually set for for %N", client);
    ...
    if (replace_primary)
    {
        LogMessage("Replacement flamethrower set for for %N", client);
...
int IsSwarmActive()
{
    for (int i = 0; i < MAXROCKETS; i++)
    {
        ...
        if (index != INVALID_ENT_REFERENCE)
        {
            ...
            if (g_RocketClass[class].spawnmult > 1)
            {
                return true;
            }
        }
...
** -------------------------------------------------------------------------- */
int GetRocketSlot(bool isSwarm = false)
{
    if (IsSwarmActive() && !isSwarm)
    {
        return -1;
    }
    ...
    if (!isSwarm && rocketCount >= g_max_rockets_dynamic)
    {
        return -1;
    }
...
void CreateSprite(const char[] parent, const char[] sprite)
{
    int ent_sprite = CreateEntityByName("env_sprite");
    if (ent_sprite)
    {
        char strEntityName[64];
        Format(strEntityName, sizeof(strEntityName), "rocket_sprite_%i", ent_sprite);

        DispatchKeyValue(ent_sprite, "classname", "env_sprite");
        DispatchKeyValue(ent_sprite, "model", sprite);
        DispatchKeyValue(ent_sprite, "targetname", strEntityName);
        DispatchKeyValue(ent_sprite, "parentname", parent);
        DispatchKeyValue(ent_sprite, "spawnflags", "1");
        DispatchKeyValue(ent_sprite, "rendermode", "1");
        DispatchKeyValue(ent_sprite, "renderfx", "17");
        DispatchKeyValue(ent_sprite, "rendercolor", "255 255 255");

        DispatchSpawn(ent_sprite);

        SetVariantString(parent);
        AcceptEntityInput(ent_sprite, "SetParent");
        SetVariantFloat(0.1);
        AcceptEntityInput(ent_sprite, "SetScale");
        SetEntProp(ent_sprite, Prop_Send, "m_bWorldSpaceScale", 0);
        SetEntityMoveType(ent_sprite, MOVETYPE_ISOMETRIC);

        float zeroVec[3] = {0.0, 0.0, 0.0};
        SetEntPropVector(ent_sprite, Prop_Send, "m_vecOrigin", zeroVec);
    }
}

int SpawnRocket(int class, int rocketTeam, float fPosition[3], float fAngles[3], bool isSwarm = false)
{
    if (!g_isDBmap)
    {
        return -1;
    ...
    int rIndex = GetRocketSlot(isSwarm);
    ...
    bool isSwarmClass = g_RocketClass[class].spawnmult > 1;
    ...
    if (iEntity && IsValidEntity(iEntity))
    {
        ...
        char strParent[64];
        Format(strParent, sizeof(strParent), "rocket_%i", rIndex);
        DispatchKeyValue(iEntity, "targetname", strParent);

        // Fetch spawn point's location and angles.
        float fDirection[3], fVelocity[3];
        ...
        if (isSwarmClass)
        {
            if (skin_color == 1)
            {
                DispatchKeyValue(iEntity, "rendercolor", "255 100 100");
            }
            else
            {
                DispatchKeyValue(iEntity, "rendercolor", "100 100 255");
            }
        }
        ...
        g_RocketClass[class].GetSprite(spritePath, PLATFORM_MAX_PATH);
        CreateSprite(strParent, spritePath);
        ...
        SDKHook(iEntity, SDKHook_Touch, OnSwarmTouch);
        ...
        if (!isSwarm)
        {
            if (isSwarmClass)
            {
                ...
                EmitSoundAllDB(rsnd_spawn, rIndex, false);
            }
            else
            {
...
public void AttachTrail(int rIndex)
{
    ...
    DispatchKeyValue(trail, "targetname", strTargetName);
...
public Action OnStartTouch(int entity, int other)
{
    ...
    int otherRef = EntIndexToEntRef(other);
    int otherIndex = GetRocketIndex(otherRef);

    // If the other entity is a swarm rocket, do not collide at all. Handled by OnSwarmTouch
    if (rIndex != -1 && otherIndex != -1 && g_RocketClass[g_RocketEnt[otherIndex].class].spawnmult > 1)
    {
...
public Action OnSwarmTouch(int entity, int other)
{
    int ref = EntIndexToEntRef(entity);
    int rIndex = GetRocketIndex(ref);
    int otherRef = EntIndexToEntRef(other);
    ...
    // If the other entity is a swarm rocket, do not collide and teleport away from each other
    if (rIndex != -1 && otherIndex != -1 && g_RocketClass[g_RocketEnt[otherIndex].class].spawnmult > 1)
    {
        float vOtherOrigin[3];
        GetEntPropVector(other, Prop_Data, "m_vecOrigin", vOtherOrigin);
        int dir = GetRandomInt(0, 2);
        int dirPosNeg = GetRandomInt(0, 1);
        vOtherOrigin[dir] += 4.0 * (dirPosNeg == 0 ? 1 : -1);
        TeleportEntity(other, vOtherOrigin, NULL_VECTOR, NULL_VECTOR);
        return Plugin_Handled;
    }
...
public Action OnTouch(int entity, int other)
{
    ...
    EmitSoundAllDB(rsnd_bounce,rIndex,true,false);
    ...
    for (int i = 0; i < MAXROCKETS; i++)
    {
        ...
        else if (rDeflects > g_RocketEnt[i].deflects)
        {
            ...
            if (rTeam == TEAM_RED)
            {
                ...
                if (g_RocketClass[class].spawnmult > 1)
                {
                    DispatchKeyValue(index, "rendercolor", "255 100 100");
                }
            }
            else
            {
                ...
                if (g_RocketClass[class].spawnmult > 1)
                {
                    DispatchKeyValue(index, "rendercolor", "100 100 255");
                }
            }
            //End fix
            ...
            // Spawn additional swarm rockets
            if (g_RocketClass[class].spawnmult > 1)
            {
                float fPosition[3];
                int original = EntRefToEntIndex(g_RocketEnt[i].entity);
                GetEntPropVector(original, Prop_Send, "m_vecOrigin", fPosition);

                for (int s = 0; s < g_RocketClass[class].spawnmult - 1; s++)
                {
                    // SpawnChildRocket(original, i, class);

                    fPosition[2] += 0.0;

                    int newIndex = SpawnRocket(class, rTeam, fPosition, fAngles, true);

                    // If spawning did not fail
                    if (newIndex != -1)
                    {
                        g_RocketEnt[newIndex].owner = g_RocketEnt[i].owner;
                        g_RocketEnt[newIndex].deflects = g_RocketEnt[i].deflects;
                        g_RocketEnt[newIndex].speed = g_RocketEnt[i].speed;
                        // g_RocketEnt[newIndex].target = g_RocketEnt[i].target;
                        g_RocketEnt[newIndex].SetDirection(fDirection);

                        g_RocketEnt[newIndex].target = SearchTarget(newIndex);

                        int newEntity = EntRefToEntIndex(g_RocketEnt[newIndex].entity);
                        SetEntProp(newEntity, Prop_Send, "m_iDeflected", g_RocketEnt[newIndex].deflects + 1);
                        CreateTimer(g_RocketClass[class].deflectdelay, EnableHoming, newIndex);
                    }
                }
            }
        }
        ...
        if (!g_RocketEnt[i].keepdir)
        {
            ...
            if (g_RocketClass[class].size > 0.0 && g_RocketClass[class].sizeinc > 0.0)
            {
                ...
                float mins[3], maxs[3];
                mins[0] = -g_RocketClass[class].hitboxX * (1.0 + g_RocketClass[class].sizeinc * g_RocketEnt[i].deflects);
                mins[1] = -g_RocketClass[class].hitboxY * (1.0 + g_RocketClass[class].sizeinc * g_RocketEnt[i].deflects);
                mins[2] = -g_RocketClass[class].hitboxZ * (1.0 + g_RocketClass[class].sizeinc * g_RocketEnt[i].deflects);

                maxs[0] = g_RocketClass[class].hitboxX * (1.0 + g_RocketClass[class].sizeinc * g_RocketEnt[i].deflects);
                maxs[1] = g_RocketClass[class].hitboxY * (1.0 + g_RocketClass[class].sizeinc * g_RocketEnt[i].deflects);
                maxs[2] = g_RocketClass[class].hitboxZ * (1.0 + g_RocketClass[class].sizeinc * g_RocketEnt[i].deflects);

                SetEntPropVector(index, Prop_Send, "m_vecMins", mins);
                SetEntPropVector(index, Prop_Send, "m_vecMaxs", maxs);
            }
...
public CreateExplosion(rIndex, rEntity)
{
    ...
    for (int iClient = 1; iClient <= MaxClients; iClient++)
    {
        if (IsValidAliveClient(iClient) && GetClientTeam(iClient) != iTeam)
        {
            ...
            float radius = g_RocketClass[class].exp_radius + g_RocketClass[class].exp_radiusinc * g_RocketEnt[rIndex].deflects;

            if (fDistanceToShockwave < radius)
            {
                float fImpulse[3], fFinalPush, fFinalDamage;
                ...
                if (fDistanceToShockwave < g_RocketClass[class].exp_fallof)
                {
                    ...
                    fFinalDamage = g_RocketClass[class].exp_damage;
                }
                else
                {
                    float fImpact = (1.0 - ((fDistanceToShockwave - g_RocketClass[class].exp_fallof) / ( radius - g_RocketClass[class].exp_fallof)));

                    fFinalPush   = fImpact * g_RocketClass[class].exp_push;
                    fFinalDamage = fImpact * g_RocketClass[class].exp_damage;
                }
                ScaleVector(fImpulse, fFinalPush);

                fPlayerPosition[2] += 2.0;

                TeleportEntity(iClient, fPlayerPosition, NULL_VECTOR, fImpulse);
                SetEntPropVector(iClient, Prop_Data, "m_vecAbsVelocity", fImpulse);
                ApplyDamage(iClient, rEntity, g_RocketEnt[rIndex].owner, fFinalDamage);
            }
        }
    }
...
** -------------------------------------------------------------------------- */
public ApplyDamage(iClient, rEntity, iAttacker, float fFinalDamage)
{
    if (IsValidAliveClient(iClient) && iAttacker >= 0 && iAttacker <= MaxClients)
    {
        // SlapPlayer(iClient, 0, true);
        SDKHooks_TakeDamage(iClient, EntRefToEntIndex(rEntity), iAttacker, fFinalDamage, DAMAGEBITS_DEFLECTED_ROCKET, -1);
    }
}
...
** -------------------------------------------------------------------------- */
void EmitSoundAllDB(int rocketsnd, int rIndex, bool fromEntity, bool playTwice = true)
{
    int index = EntRefToEntIndex(g_RocketEnt[rIndex].entity);
    if (index == INVALID_ENT_REFERENCE)
    {
    ...
    if(fromEntity)
    {
        EmitSoundToAll(strFile, index, _, SNDLEVEL_GUNFIRE,_,SNDVOL_NORMAL);
        if(StrContains(strFile, ".mp3", false) && playTwice)
        {
            EmitSoundToAll(strFile, index, _, SNDLEVEL_GUNFIRE,_,SNDVOL_NORMAL);
        }
    }
    ...
    {
        EmitSoundToAll(strFile, _, _, SNDLEVEL_GUNFIRE,_,SNDVOL_NORMAL);
        if(StrContains(strFile, ".mp3", false) && playTwice)
        {
            EmitSoundToAll(strFile, _, _, SNDLEVEL_GUNFIRE,_,SNDVOL_NORMAL);
        }
...
void StopSoundAllDB(const char[] strFile)
{
    for (int client = 1; client <= MaxClients; client++)
    {
        if (IsClientInGame(client))
        {
            StopSound(client, SNDCHAN_AUTO, strFile);
            StopSound(client, SNDCHAN_AUTO, strFile);
            StopSound(client, SNDCHAN_AUTO, strFile);
            StopSound(client, SNDCHAN_AUTO, strFile);
        }
